...
<div class="m m_b14">
    <div class="w_300 f_right">
        <dl class="k_dl4 m_b14">
            <dt>
            <h1>我的关注</h1><em></em><?php if (count($my_mark_school_news_list) > 5) { ?><a style="color: #5d8080" href="<?= Yii::app()->createUrl('news/index', array('alumni_id' => $my_mark_alumnis, 'category_id' => 1)); ?>" target="_blank">更多</a><?php } ?>
            </dt>
            <dd class="ar_sho">
                <ul>
                        ...
                        <?php foreach ($my_mark_school_news_list as $value) { ?>
                            <li><i></i>
                                <h1 style="width:200px;">
                                    ...
                                    <a href="<?= Yii::app()->createUrl('news/view', array('id' => $value['id'])); ?>" target="_blank" title="<?= '[' . $value['name'] . ']' . $value['title']; ?>"><?= $value['title']; ?></a>
                                </h1>
                            </li>
                        <?php } ?>
                    <?php } else { ?>
        ...
        <dl class="k_dl4 m_b14">
            <dt>
            <h1>我校新闻</h1><em></em><?php if (count($my_school_news_list) > 5) { ?><a style="color: #5d8080" href="<?= Yii::app()->createUrl('news/index', array('alumni_id' => $alumni_info->id, 'category_id' => 1)); ?>" target="_blank">更多</a><?php } ?>
            </dt>
            <dd class="ar_sho">
                <ul>
        ...
        <dl class="k_dl4 m_b14">
            <dt>
            <h1>同省新闻</h1><em></em><?php if (count($city_news_list) > 5) { ?><a style="color: #5d8080" href="<?= Yii::app()->createUrl('news/index', array('alumni_id' => $city_alumni_list_ids, 'category_id' => 1)); ?>" target="_blank">更多</a><?php } ?>
            </dt>
            <dd class="ar_sho">
                <ul>
    ...
    <div class="ar_sho m_l210 w_580">
        ...
        <div class="art_showdiv">
            ...
            <div class="alumni_tit" style="background:none;">
                <a href="javascript:void(0);" class="attention" id="user_marked"><img src="<?= Yii::app()->request->baseUrl; ?>/static/images/ico_16.png" /><span id="mark_if"><?= $is_marked ? '取消关注' : '关注'; ?></span><?= $event->alumni->name; ?></a>
            </div>
            ...
            <div class="art_comment">
                        ...
                        foreach ($comments_list as $value) {
                            ...
                            <dl>
                                ...
                                <dd>
                                    ...
                                    <div class="content"><a style="float: right; margin-right: 30px" href="javascript:void(0);" onclick="quote_comment(<?= $floor; ?>, <?= Yii::app()->user->isGuest ? Yii::app()->user->isGuest : 0; ?>, '<?= Yii::app()->createUrl('site/ajaxLogin', array('uid' => $this->userInfo->id)); ?>')">引用</a></div>
                                </dd>
                            </dl>
                            <?php
                            $floor++;
                ...
                <dl>
                    ...
                    <dd>
                        ...
                        <?php } else { ?>
                            登陆以后才能评论，请先&nbsp;&nbsp;&nbsp;<a href="javascript:;" onclick="ajaxLoginForm('<?= Yii::app()->createUrl('site/ajaxLogin', array('uid' => $this->userInfo->id)); ?>');">登录</a>
    ...
    $(document).ready(function() {
        $('#put_comment').click(function() {
            ...
            $.ajax({
                ...
                data: {'content': $('#comment').val(), 'quote_comment': $('#quote_comment').val(), 'is_anonymity': is_anonymity, 'event_id':<?= $event['id']; ?>},
        ...
        $('#user_marked').click(function() {
            ...
            if (is_loginer) {
                ...
                if ($('#mark_if').html() == '关注') {
                    $.ajax({
                        type: 'post',
                        url: '<?= Yii::app()->createUrl('alumni/mark', array('id' => $event['alumni_id'])); ?>',
                        success: function(data) {
                            $('#mark_if').html('取消关注');
                        }
                    });
                    ...
                    $.ajax({
                        type: 'post',
                        url: '<?= Yii::app()->createUrl('alumni/markDelete', array('id' => $event['alumni_id'])); ?>',
                        success: function(data) {
                            $('#mark_if').html('关注');
                        }
                    });
