...
class WeixinController extends Controller {
    ...
    public function actionIndex() {
        ...
        $criteria->select = 'id,title,created_at,alumni_id,hits_num';
        $criteria->condition = $where;

        $count = Weixin::model()->count($criteria);
        $pages = new CPagination($count);
        $pages->pageSize = 15;
        $pages->applyLimit($criteria);
        $records = Weixin::model()->findAll($criteria);
    ...
    public function actionView($id, $uid = null) {
        ...
        $news->hits_num = $news->hits_num + 1;
        $view['news'] = $news;
        $news->save();

        //新闻的评论列表
        $view['comments_list'] = array();

        //下一篇新闻
        $next_news = Yii::app()->db->createCommand()
            ->select('id,title,alumni_id')
            ->from('weixin')
            ->where('category_id=' . $news->category_id . ' AND created_at <= "' . $news->created_at . '" AND id != ' . $id)
        ...
        if ($user_info) {
            ...
            $my_school_news_list = Yii::app()->db->createCommand()
                ->select('n.id,n.title,n.alumni_id, a.name as name')
                ->from('weixin as n')
